...
\begin{figure}
  ...
  \caption{Structure of interpretation loop in most common interpreters.  Program addresses of Lines 1, 2, and 4 are per interpreter.  High-level control flow changes at Lines 4, 9, and 10 are detected at runtime by watchpoints on HLPC address identified at Line 2.}
...
%
The boundaries between program statements are a high-level property of the language, which may not reflect in the low-level implementation.  Even at the high level, statement boundaries may be ambiguous due to functional constructs, such as lambda functions and list comprehensions, or data definitions mixed with control flow, such as class definitions in Python.

In practice, however, most interpreters follow a common structure that \emph{can be detected automatically}.
%
Instead of directly interpreting the source program, interpreters first compile it to an intermediate bytecode representation.  At that level, the statement boundaries are clear: each bytecode instruction is a statement.  Each function in the program gets its own bytecode block.  When a function is called, the interpreter pushes a frame on its high-level stack, which includes a high-level program counter variable that points inside the function bytecode buffer.
%
We confirmed these patterns by inspecting the source code of the most popular interpreters for Python, JavaScript, Ruby, PHP, and Lua.

For each interpreter binary, \chef reconstructs a model consisting of three binary locations (breakpoints).

At runtime, the breakpoints are used to maintain the high-level stack and the current high-level program counter.

\paragraph{Model Reconstruction in \chef}

\chef performs model reconstruction by observing all memory accesses in the interpreter when running a special ``calibration'' script.

The role of the calibration script is to create HLPC update patterns that are distinguishable from other memory accesses.

\chef uses a linear sequence of instructions to create a linear HLPC update pattern.

The calibration script is written once for each language, and executed once for each interpreter binary.
