...
We illustrate the program-facing workflow using the Python example in Figure~\ref{fig:chef:running-example}.  The program under test receives an e-mail address string and raises an exception if the address is malformed.


\begin{figure}
  \centering
  \includegraphics[width=2.2in]{figures/chef/running-example}
  \caption{Example of Python code that validates an e-mail address provided as a string.}
  \label{fig:chef:running-example}
\end{figure}

Consider the Python example in Figure~\ref{fig:chef:running-example}, which receives an e-mail string as input and 
