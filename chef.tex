...
%
On the other hand, it includes the communication interface with the execution strategy, which decides which of the available program states in the execution tree to be run next (see Section~\ref{sec:intro:symbex}).

Figure~\ref{fig:xxx} shows the architecture of \chef, which consists of a fixed part and a pluggable part.
%
...
The inner low-level symbolic execution engine operates with machine-level abstractions.

The outer interface exposes high-level abstractions.
...
The choice of low-level execution paths is governed by \chef's strategy subsystem.

The low-level states maintained by the low-level engine are reported to a state adapter, which constructs the ``high-level view'' of the execution.

The state adapter decides which high-level states to prioritize, according to a pluggable high-level strategy.

In turn, the low-level strategy guides the exploration in the interpreter according to the selected high-level state.
