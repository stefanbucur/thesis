...
%
Finally, the high-level strategy communicates its choice to the low-level strategy, which takes it into account when selecting the next state for the low-level engine.

This state selection process hides the low-level symbolic execution engine from the outer interface, hence preserving the high-level abstractions.

The abstract high-level states only contain the current HLPC and, optionally, a path condition.
%
