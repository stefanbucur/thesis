...
\section{\chef as Symbolic Execution Engine Adapter}
...
Broadly speaking, a symbolic execution engine interface consists of two parts.
%
On the one hand, the engine receives the program and a test driver, and outputs a set of test cases.
%
On the other hand, the engine interacts with the execution strategy that decides which program state to execute next, according to the exploration goal.
%
\chef connects the two parts of the inner and outer engine interfaces, as follows.

Figure~\ref{fig:xxx} shows the architecture of \chef.

Broadly speaking, a symbolic execution engine receives the program, a symbolic input for the program, and a goal in the form of a search strategy; the engine outputs test cases that exercise different paths in the program.
