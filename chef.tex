...
%
On the other hand, it includes the communication interface with the execution strategy, i.e., the component that decides which of the available program states in the execution tree to be run next (see Section~\ref{sec:intro:symbex}).

Figure~\ref{fig:chef:arch} shows the architecture of \chef, which consists of a fixed part and a pluggable part.
%
The fixed part includes the low-level symbolic execution engine (S2E), its state selection strategy, and a component that converts low-level execution states in the interpreter to high-level execution states in the target program.
%
...
%
During symbolic execution, the low-level execution strategy is informed of all execution states that fork in the execution tree, and decides which of them to execute next.

The outer interface exposes high-level abstractions (the outer block in Figure~\ref{fig:chef:arch}).
%
...
%% This chapter presents the techniques \chef employs to fullfill the adapter role, which is broadly divided in two parts:
%% %
%% (1) maintaining a view of high-level execution paths and states, obtained from the low-level interpreter execution (Section~\ref{sec:chef:hlsymbex}), and (2) translating high-level path prioritization goals back to choosing interpreter paths (Section~\ref{sec:chef:strategies}).


\section{From Low-level to High-level Symbolic Execution Trees}

The main function of \chef is to choose the low-level execution paths in the interpreter.
%
This is handled by \chef's strategy subsystem.

The choice is done in a feedback loop.

First, the low-level interpreter states are reported to a \emph{state adapter}, which constructs the ``high-level view'' of the interpreter execution.

Second, a pluggable high-level strategy decides which high-level states provided by the state adapter to prioritize.

Finally, back at the interpreter, the low-level strategy guides its exploration according to the selected high-level state.
...
\section{Trading Precision and Performance with State Completeness Models}
...
In partial mode, a high-level state is allowed to be anywhere on the completed and partial segments.

In partial mode, \chef cannot decide whether a high-level state would branch at the current HLPC.

The advantage of a partial state is that it can progress faster, as only one low-level state needs to be advanced.
...
In complete mode, a high-level state can only be anywhere on the completed path segment.


\section{The Language Interpreter}
