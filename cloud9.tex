...
%
In this chapter, we \cnine, a symbolic execution platform for system programs that is based on a symbolic model of most essential operating system abstractions: threads, process management, sockets, pipes, polling, etc. (Section~\ref{sec:cloud9:primitives}).
%
...
Ideally, a symbolic model would be implemented as guest code directly run by the SEE.
%
With this approach, the model would implicitly handle symbolic data, as opposed to explicitly build symbolic expressions using the internal engine API.
%
The resulting implementation would be significantly shorter and less prone to errors.
...
%
On the other hand, implementing the model inside the symbolic execution engine would constitute a significant engineering effort.


We designed a minimal yet general ``symbolic system call'' interface to the \cnine SEE, which provides the essential building blocks for thread context switching, address space isolation, memory sharing, and sleep operations.
%
...
In order to support the POSIX interface, we augmented \klee with two major features:
%
multiple address spaces per state and support for scheduling threads and processes.  This functionality is accessed by model code through the symbolic system call interface (Table~\ref{table:cloud9:primitives}).  Additional models of non-POSIX environments can be built using this interface.
...
\cnine implements a cooperative scheduler:
%
...
We used the SEE system call interface to build a model for most common POSIX APIs.

In this section, we describe the key design decisions involved in building the \cnine POSIX model, and we illustrate the use of the symbolic system call interface.
%
This is of particular interest to readers who wish to build additional models on top of the \cnine symbolic system call interface.
...
\cnine inherits most of the semantics of the file model from \klee.
%
