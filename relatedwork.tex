...
%
The last part of the chapter covers the use of symbolic execution for interpreted languages, which is a particular focus of our work (Section~\ref{sec:relwork:iterplang}).
...
A major limitation of the early symbolic execution engines was the lack of proper support for reasoning about symbolic expressions.  Programs commonly use binary arithmetic and access memory locations at arbitrary offsets.  However, DART and CUTE used a solver for linear integer constraints, which limited the parts of the code that could be reasoned about symbolically.

Subsequent symbolic execution engines addressed this problem by employing high-performance specialized constraint solving utilities, such as Z3~\cite{Z3}, STP~\cite{stp}, or CVC~\cite{cvc}.  These solvers can reason efficiently about a large set of operations commonly encountered in program execution, such as bit-wise arithmetic and array manipulation.
%
...
DART and CUTE implemented concolic execution at the C source code level, by using CIL~\cite{cil} to instrument the code with additional statements that maintain the symbolic state as the program executes.
...
\begin{figure}
  ...
  \caption{Environment problem in the related work.}
